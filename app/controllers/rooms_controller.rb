class RoomsController < ApplicationController
  ...
  def create
    @ticket = Ticket.find(params[:ticket_id])
    room = Room.find_by(ticket_id: @ticket.id)
    if room.present?
      redirect_to user_room_messages_path(@ticket.user_id, room.id)
    else
      @room = Room.new(name: params[:name], ticket_id: @ticket.id)
      if @room.save
        RoomUser.create(room_id: @room.id, user_id: current_user.id)
        RoomUser.create(room_id: @room.id, user_id: @ticket.user_id)
        redirect_to user_room_messages_path(@ticket.user_id, @room.id)
      else
        render :create
      end
